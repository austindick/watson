# Design Toolkit Agent Contract Spec

## Overview

This is the canonical enumeration of all Design Toolkit agents. It is the source of truth for dispatch parameters, dispatch modes, and output paths. Agent YAML frontmatter mirrors these entries exactly — the two are a mirrored pair, not independently maintained documents. When an agent is ported or built in a later phase, its frontmatter is copied from its registry row here.
...
| librarian | shared/librarian.md | background | mode (enum: generate \| update), sourcePaths (string[] — paths Librarian scans), outputBookPath (string — target library/book/ directory) | library/{book}/BOOK.md + CHAPTER.md files |
...
- **interaction** — always background. Receives all required context via parameters (`interactionContext` from discuss, or `quietMode=true` in loupe parallel dispatch).

### Interaction Agent Note

The interaction agent always runs as background. When `interactionContext` is provided (from discuss), it writes INTERACTION.md from that context. When dispatched by loupe in parallel mode, `quietMode=true` is always set.

---

## Error Contract

Agents return raw errors to the dispatching subskill. The subskill translates errors into non-technical, designer/PM-friendly language before surfacing them to the user.
