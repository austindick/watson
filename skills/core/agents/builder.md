...
4. Use the Edit tool for section replacement, NOT the Write tool — Write overwrites the entire file. Exception: when `sectionType` is `"page-container"`, use the Write tool to create the initial page scaffold (see Page-Container Mode below)
...
- `sectionType` (string, optional) — when `"page-container"`, triggers wrapper generation mode instead of section replacement (see Page-Container Mode below); omit or pass `undefined` for normal section builds
- `portalType` (string, optional) — `"Retailer"` or `"Brand"`, used to select portal template values from page-templates library chapter; default `"Retailer"` when not specified; only used in page-container mode
- `childSections` (string[], optional) — names of child sections to create placeholder stubs for; only used in page-container mode; matches the section names from the decomposer or surface-resolver output (excluding the page-container entry itself)
...
- **Page-container mode exception:** a new file is created at `targetFilePath` using the Write tool (initial page scaffold — see Page-Container Mode below)
...
**Early return check:** If `sectionType` is `"page-container"`, skip Steps 4 and 5 entirely — there is no existing section to locate. Proceed directly to Page-Container Mode.
...
When `sectionType` is `"page-container"`, the builder generates the initial page scaffold file instead of editing an existing section. This is the first step of the prototype build — it creates the file that all subsequent section builders will edit into.

### PC Step 1: Read page-templates chapter

From `libraryPaths`, locate the file whose path contains `page-templates`. Read it. Extract the template block matching `portalType` (default `"Retailer"` if not specified). From the matching portal section, extract:
- Background color token
- Padding tokens (top, bottom, left, right)
- Content max-width value
- Inter-section spacing token (gap between child sections)

### PC Step 2: Read LAYOUT.md for page-container

Read `layoutPath` — this is the LAYOUT.md generated by the layout agent for the page-container section. Its Annotated CSS may provide Figma-derived overrides to the portal template values. For each property in the LAYOUT.md Annotated CSS: use the LAYOUT.md token. For properties absent from LAYOUT.md: fall back to portal template values.

### PC Step 3: Generate wrapper structure

Build a React component (or JSX fragment) for the page scaffold:

1. **Outer wrapper div** — token-styled container using the merged values from PC Steps 1 and 2:
   ```tsx
   <div
     style={{
       backgroundColor: "var(--color-background-primary)",  /* portal-template */
       paddingTop: "var(--spacing-layout-xl)",               /* portal-template: 24px */
       paddingBottom: "var(--spacing-layout-xl)",
       paddingLeft: "var(--spacing-layout-2xl)",
       paddingRight: "var(--spacing-layout-2xl)",
       maxWidth: "1280px",                                   /* portal-template: no token */
       display: "flex",
       flexDirection: "column",
       gap: "var(--spacing-section-lg)",                     /* portal-template: 48px */
     }}
   >
   ```

2. **Named placeholder stubs** — for each name in `childSections[]`, emit a stub using the most reliable Edit-tool-compatible pattern:
   ```tsx
   {/* --- [SectionName] --- */}
   <div data-section="[sectionName]">
     {/* Section: [SectionName] — replaced by section builder */}
   </div>
   ```
   - `[sectionName]` is the child section name as-is (preserves original casing)
   - `data-section` attribute is lowercase-kebab-case of the section name (e.g., "Product Grid" → `data-section="product-grid"`)
   - The comment `{/* --- [SectionName] --- */}` immediately precedes the div so a subsequent builder can locate the stub via either the comment OR the `data-section` attribute
   - Stubs are ordered in the same order as `childSections[]`

### PC Step 4: Write scaffold using Write tool

Use the Write tool (not Edit) to create `targetFilePath`. This is the ONE exception to Constraint 4 — the file does not yet exist, and there is no existing section to replace.

The written file must be a valid TSX file importable by the Playground:
- Export the page component as default or named export (match the `sectionScope` parameter name)
- Include only essential imports (React, no Slate imports needed in the scaffold itself)
- All tokens via inline style or a minimal CSS class; no external CSS files needed for the scaffold

### PC Step 5: Compile verification

Run the same compile verification as Step 9 of the normal flow. The scaffold must compile cleanly before returning.
