---
name: session-init
description: Branch operations, blueprint scaffold, and session management for /play
---

# Reference: Session Init
...
Also handles single-file prototype promotion: if a prototype exists as `MyPage.tsx`, session-init converts it to `MyPage/index.tsx` and creates `MyPage/blueprint/`.
...
Invoked when `operation` is not "branch-list" — i.e., SKILL.md Path A dispatched session-init with `prototype_name` and `slug`.
...
Invoked when `operation` is "branch-list" — i.e., SKILL.md Path B dispatched session-init.
